...
### 🚨 components[].type은 반드시 "template"으로 고정

**bdc-web과의 호환성을 위해 모든 템플릿의 `components[].type`은 `"template"`으로 통일합니다.**
...
- bdc-web에서는 별도 ID로 템플릿을 구분하므로, type은 항상 `"template"`
- 새 템플릿 JSON 생성 시 처음부터 `"type": "template"` 사용
- 로컬 개발 환경에서는 JSON 최상위 `id` 값으로 렌더러 구분

참고: [WARNNING.md](./WARNNING.md)

---

### 🏷️ 템플릿 이름(name) 작성 규칙

**새 템플릿 생성 시 사용자가 지정한 이름을 `name` 필드에 입력합니다.**

```json
// 사용자 요청: "새로운 템플릿 만들려고 하는데 name은 '따뜻한 시선'이야"
{
  "id": "wedding-card-012",
  "version": "4.0.0",
